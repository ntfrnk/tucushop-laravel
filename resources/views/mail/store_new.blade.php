...
<p><b>Nombre:</b><br> {{ $info->user->admins->first()->store->name }}</p>
<p><b>Usuario:</b><br> {{ '@'.$info->user->admins->first()->store->alias }} </p>
<p><b>Descripción:</b><br> {{ $info->user->admins->first()->store->description }}</p>
<p><b>Creado:</b><br> {{ $info->user->admins->first()->store->created_at }}</p>
</div>
