						...
						<tbody>
							@foreach($messages->sortByDesc('answers') as $message)
								...
								<tr>
									...
									<td class="a-right">
										<a href="{{ route('user.message.read', ['message_id' => $message->id]) }}" class="btn btn-sm btn-outline-secondary"><i class="fa fa-eye"></i><span class="d-none d-md-block"> Ver mensaje</span></a>
										<a href="javascript:;" onclick="confirm_open_link('¿Estás seguro de que quieres eliminar este mensaje?', '{{ route('user.message.delete', ['message_id' => $message->id]) }}')" class="btn btn-sm btn-outline-danger"><i class="fa fa-times"></i><span class="d-none d-md-block"> Eliminar</span></a>
									</td>
								</tr>
