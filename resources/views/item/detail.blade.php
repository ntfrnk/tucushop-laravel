			...
			@if($item_disabled != 0)
				...
				<div class="col-md-6 item-col-detail">
					...
					<div class="row marT0 marB20">
						<div class="col-md-12">
							<a rel="nospinner" class="fb-share-button" 
								data-href="{{ Request::url() }}" 
								data-layout="button">
							</a>
							<a rel="nospinner" href="http://pinterest.com/pin/create/button/?url={{ urlencode(Request::url()) }}&amp;media={{ urlencode(route('home').'/storage/items/lg/'.$item->photos->first()->file_path) }}" class="btn btn-primary btn-sm btn-pt f18">
								<i class="fab fa-pinterest"></i>
							</a>
							<a rel="nospinner" href="whatsapp://send?text={{ urlencode($item->name.': '.Request::url()) }}" class="badge badge-success text-white btn-wp">
								<i class="fab fa-whatsapp f13 block f-left"></i> <span class="inline-block f11 fw600 lh10">Enviar</span>
							</a>
							<a rel="nospinner" href="https://twitter.com/intent/tweet?text={{ urlencode($item->name) }}&url={{ urlencode(Request::url()) }}" class="badge badge-info text-white btn-wp">
